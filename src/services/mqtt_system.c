...
/* Pinos para atuadores */
#define GPIO_LAMP_PIN 18
#define GPIO_AC_PIN 19

/* Tempo para desligar AC se temperatura ficar abaixo de 20 por 10 minutos */
#define AC_OFF_AFTER_MS (10 * 60 * 1000ULL)
...
static TaskHandle_t s_task_sensor_lum = NULL;
static TaskHandle_t s_task_sensor_temp = NULL;
...
static void sensor_luminosity_task(void *pvParameters);
static void sensor_temperature_task(void *pvParameters);
...
static void handle_incoming_luminosity(const char *payload, int len);
static void handle_incoming_temperature(const char *payload, int len);

/* Estado para monitoramento de temperatura */
static uint64_t s_temp_below_start_ms = 0;
...
esp_err_t mqtt_system_init(void)
{
    ...
    /* Fase 3.5: Inicializar GPIO de controle */
#ifndef CONFIG_QEMU_MODE
    init_gpio();
#else
    ESP_LOGW(TAG, "GPIO ignorado (modo QEMU)");
#endif
...
esp_err_t mqtt_system_shutdown(void)
{
    ...
    if (s_task_sensor_lum)
    {
        vTaskDelete(s_task_sensor_lum);
        s_task_sensor_lum = NULL;
    }
    if (s_task_sensor_temp)
    {
        vTaskDelete(s_task_sensor_temp);
        s_task_sensor_temp = NULL;
    }
...
#ifndef CONFIG_QEMU_MODE
    /* Tasks dos sensores simulados */
    ret = xTaskCreate(sensor_luminosity_task, "SensorLum",
                      2048, NULL, 5, &s_task_sensor_lum);
    if (ret != pdPASS)
    {
        ESP_LOGE(TAG, "  Falha ao criar task sensor luminosidade");
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "  Task de sensor luminosidade criada");

    ret = xTaskCreate(sensor_temperature_task, "SensorTemp",
                      2048, NULL, 5, &s_task_sensor_temp);
    if (ret != pdPASS)
    {
        ESP_LOGE(TAG, "  Falha ao criar task sensor temperatura");
        return ESP_FAIL;
    }
    ESP_LOGI(TAG, "  Task de sensor temperatura criada");
#else
    ESP_LOGI(TAG, "  Tasks de sensores ignoradas (modo QEMU)");
#endif
...
/* Inicializa GPIOs de controle (lampadas / AC) */
static void init_gpio(void)
{
#ifndef CONFIG_QEMU_MODE
    gpio_reset_pin(GPIO_LAMP_PIN);
    gpio_reset_pin(GPIO_AC_PIN);

    gpio_set_direction(GPIO_LAMP_PIN, GPIO_MODE_OUTPUT);
    gpio_set_level(GPIO_LAMP_PIN, 0);

    gpio_set_direction(GPIO_AC_PIN, GPIO_MODE_OUTPUT);
    gpio_set_level(GPIO_AC_PIN, 0);
#endif
}

/* Conversores e handlers para mensagens de sensores externos */
static void handle_incoming_luminosity(const char *payload, int len)
{
    ...
    char buf[64];
    int rlen = len < (int)sizeof(buf) - 1 ? len : (int)sizeof(buf) - 1;
    memcpy(buf, payload, rlen);
    buf[rlen] = '\0';

    float val = strtof(buf, NULL);

    ESP_LOGI(TAG, "Luminosidade recebida: %.2f", val);

#ifndef CONFIG_QEMU_MODE
    if (val < 3.0f)
    {
        gpio_set_level(GPIO_LAMP_PIN, 1);
        ESP_LOGI(TAG, "Luz ACESA (GPIO %d)", GPIO_LAMP_PIN);
    }
    else
    {
        gpio_set_level(GPIO_LAMP_PIN, 0);
        ESP_LOGI(TAG, "Luz APAGADA (GPIO %d)", GPIO_LAMP_PIN);
    }
#else
    ESP_LOGW(TAG, "Ignorando controle de GPIO (modo QEMU)");
#endif
}
...
static void handle_incoming_temperature(const char *payload, int len)
{
    ...
    ESP_LOGI(TAG, "Temperatura recebida: %.2f°C", t);

#ifndef CONFIG_QEMU_MODE
    /* Ligar ar condicionado se > 23 */
    if (t > 23.0f)
    {
        gpio_set_level(GPIO_AC_PIN, 1);
        s_temp_below_start_ms = 0; /* reset do temporizador */
        ESP_LOGI(TAG, "AC LIGADO (GPIO %d)", GPIO_AC_PIN);
        return;
    }

    /* Se temperatura abaixo de 20, começar/continuar contagem para desligar */
    uint64_t now_ms = esp_timer_get_time() / 1000ULL;

    if (t < 20.0f)
    {
        if (s_temp_below_start_ms == 0)
        {
            s_temp_below_start_ms = now_ms;
            ESP_LOGI(TAG, "Temperatura abaixo de 20°C - iniciando contagem para desligar AC");
        }
        else if ((now_ms - s_temp_below_start_ms) >= AC_OFF_AFTER_MS)
        {
            gpio_set_level(GPIO_AC_PIN, 0);
            ESP_LOGI(TAG, "Temperatura permaneceu abaixo de 20°C por 10min - AC desligado (GPIO %d)", GPIO_AC_PIN);
            s_temp_below_start_ms = 0; /* evitar repetir desligamento */
        }
    }
    else
    {
        /* temperatura entre 20 e 23 - apenas resetar se estiver abaixo */
        s_temp_below_start_ms = 0;
    }
#else
    ESP_LOGW(TAG, "Ignorando controle de GPIO de temperatura (modo QEMU)");
#endif
}

/* Tasks simuladas de sensores */
static void sensor_luminosity_task(void *pvParameters)
{
    ESP_LOGI(TAG, "Task sensor luminosidade iniciada");

    while (1)
    {
        if (s_mqtt_connected)
        {
            int val = esp_random() % 11; /* 0..10 */
            char payload[32];
            snprintf(payload, sizeof(payload), "%d", val);
            mqtt_publish_data(TOPIC_CASA_EXTERNO_LUMINOSIDADE, payload, 0, 1, false);
            ESP_LOGD(TAG, "SensorLum publicou %s", payload);
        }
...
static void sensor_temperature_task(void *pvParameters)
{
    ESP_LOGI(TAG, "Task sensor temperatura iniciada");

    while (1)
    {
        if (s_mqtt_connected)
        {
            int val = (esp_random() % 49) - 3; /* -3..45 */
            char payload[32];
            snprintf(payload, sizeof(payload), "%d", val);
            mqtt_publish_data(TOPIC_CASA_EXTERNO_TEMPERATURA, payload, 0, 1, false);
            ESP_LOGD(TAG, "SensorTemp publicou %s", payload);
        }
                               ...
                               int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        /* Subscrições pedidas: tópicos externos de sensores */
        mqtt_subscribe_topic(TOPIC_CASA_EXTERNO_LUMINOSIDADE, 1);
        mqtt_subscribe_topic(TOPIC_CASA_EXTERNO_TEMPERATURA, 1);
        break;
    ...
    case MQTT_EVENT_DATA:
        ...
        s_stats.total_recebidas++;
        /* Roteamento simples com base no tópico recebido */
        if (event->topic_len == strlen(TOPIC_CASA_EXTERNO_LUMINOSIDADE) &&
            strncmp(event->topic, TOPIC_CASA_EXTERNO_LUMINOSIDADE, event->topic_len) == 0)
        {
            handle_incoming_luminosity(event->data, event->data_len);
        }
        else if (event->topic_len == strlen(TOPIC_CASA_EXTERNO_TEMPERATURA) &&
                 strncmp(event->topic, TOPIC_CASA_EXTERNO_TEMPERATURA, event->topic_len) == 0)
        {
            handle_incoming_temperature(event->data, event->data_len);
        }
